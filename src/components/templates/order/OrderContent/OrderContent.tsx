...
const OrderContent:React.FC = ()=>{
    ...
    const filteredBranch = useMemo(()=>{
        return Branches?.filter((branch:BranchesProps)=>branch.city_id === cityID)
    },[cityID , Branches]);
    
    const filteredInfoBranch = useMemo(()=>{
        return filteredBranch?.filter((branch:BranchesProps)=> branch.id === branchId);
                ...
                <div className="grid gap-8 lg:grid-cols-3">
                    <div className="space-y-6 lg:col-span-2">
                        ...
                        <div className="rounded-xl bg-white p-6 shadow-md">
                            ...
                            <div className="mt-4 rounded-lg bg-secondary/50 p-4 font-bold" key={branch.id}>
                                <p className="text-sm">
                                    ...
                                    <div className="flex items-start gap-2" key={menu.id}>
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between">
                                                <span>
                                                    {menu.name} ({menu.size === 'small'?'کویچک':menu.size === 'medium'?'متوسط' : menu.size === 'large'?'بزرگ':'ندارد'})
                                                </span>
                                                <span>{menu.quantity} x</span>
                                            </div>
                                            <div className="mt-0.5 text-muted-foreground">
                                                {(Number(menu.base_price) * menu.quantity).toLocaleString('fa-IR')} تومان
                                            </div>
                                        </div>
                                    </div>
                                    ))}
